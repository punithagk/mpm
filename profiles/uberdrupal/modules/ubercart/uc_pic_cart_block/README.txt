// $Id: README.txt,v 1.2 2009/04/11 11:37:41 vetkhy Exp $

Pictured cart block provides some advantages in contrast with standard
Ubercart block.
 ...
 * Optional scrolling.
 * Items count in summary is number of icons, not sum of products
   quantity (optional).

UC pic cart block was written by Dmitriy "Vetkhy" Kosolapov,
dkwrk84@gmail.com, http://vetkhytest.ucoz.ru/ for russian readers.
...
Drupal site (/sites/all/modules or /sites/all/modules/ubercart/contrib as
Ubercart contributed module), enable it in admin panel
...
IMPORTANT: if you are using caching of blocks or pages, cart block will work
only for registered users. In this case guests will see a message and links
to register, login and cart page or nothing (optional).

Note the standard cart block is not compatible with caching too.

Designers can customize block via CSS and changing the graphics used
("img" subdir). Note the filenames must be equal if you don't want
to modify code :)
