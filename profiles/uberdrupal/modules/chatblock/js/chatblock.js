 ...
 */
Drupal.behaviors.chatblockInit = function(context) {
  ...
  if (!Drupal.chatblock.inited) {
    ...
    $('#edit-chatblocktext', context).bind('keydown',function(event){
      if (
        event.which == 13
        &&
        $('#edit-chatblocktext').val().replace(/(^\s+|\s+$)/g,'') != ''
      ) {
        event.preventDefault();
 ...
 */
Drupal.behaviors.chatblockInitTimeStrings = function() {
...
Drupal.chatblock.getMessage = function() {
  $.ajax({
    type: "POST",
    dataType: "json",
    url: Drupal.settings.basePath + Drupal.settings.chatblock.viewUrl,
    data: {
...
Drupal.chatblock.response = function(jsonData) {
  ...
  if (jsonData.messages && jsonData.messages.length) {
    $.each(jsonData.messages, function(idx, message) {
      // If we did not already process this message (= unless the
      // message's id is not in our remember array):
      if ($.inArray(message.i,Drupal.chatblock.responseIds) < 0 ) {
...
Drupal.chatblock.sendMessage = function(button) {
  ...
  $.ajax({
    type: "POST",
    dataType: "json",
    url: Drupal.settings.basePath + Drupal.settings.chatblock.updateUrl,
    data: {
      'maxId': Drupal.settings.chatblock.maxId,
      'chatboxtoken': Math.random(),
